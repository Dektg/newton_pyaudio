...
while True:
    ...
    if ((mess.find('без') != -1 or mess.find('выкл') != -1) and mess.find('звук') != -1) or mess.find('тишина') != -1:
        def talk(words):
            print(words)
        print('Done')
    if mess == ('включи'):
    ...
    if 'накажи' in mess or 'дос' in mess or 'атака' in mess or 'задаюсь' in mess:
        banner = '' \
                 '         *****  *     *  *****        ******  ******* *     * ******  ******* ******\n' \
                 '        *     * **   ** *     *       *     * *     * **   ** *     * *       *     * \n' \
                 '        *       * * * * *             *     * *     * * * * * *     * *       *     *\n' \
        ...
        if mess.find('семен') != -1 or mess.find('семён') != -1:
            ...
            _phone = '79147194077'
        elif mess.find('андрей') != -1 or mess.find('андрея') != -1:
        ...
        #try:
            #try:
                #res = client.mess(mess)
                #results = next(res.results).text
                #talk('WOLFRAM-ALPHA сказоно - ')
                #talk('нашел.')
                #talk(results)
                        
            #except:
                #results = wikipedia.summary(mess, sentences=1)
                #talk('нашел.')
                #talk('в википедии сказоно - ')
                #talk(results)
